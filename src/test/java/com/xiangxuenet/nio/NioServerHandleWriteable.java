...
public class NioServerHandleWriteable implements Runnable {
   ...
   private volatile boolean started;
		...
		if(key.isValid()) {
			...
			if(key.isWritable()) {
				...
				if(att.hasRemaining()) {    //如果有数据了
					...
					key.interestOps(SelectionKey.OP_READ); //只留下读的事件
				}
			}
		}
	}
	...
	private void doWrite(SocketChannel channel,String response) throws Exception{
        ...
        //flip操作
        writeBuffer.flip();
        //解决一次写不完的数据  ,关注读写事件，把一个writeBuffer 挂载到事件上
        channel.register(selector, SelectionKey.OP_READ|SelectionKey.OP_WRITE,writeBuffer);
	}
}
